...
/**
 * Collecting the ETH back out of a finished V6 run.
 *
 * After the exit has sold every position, the proceeds sit in the bundle wallets and
 * are no use there. This gathers them.
 *
 * IT GOES THROUGH RELAY, AND THAT IS THE ENTIRE POINT OF THE MODULE. A direct sweep
 * would undo the whole run: every bundle wallet bought in public, and if all of them
 * then send ETH straight to one address, anyone reading the chain sees the buyers
 * funnelling into a single wallet — the link the Relay hop avoided on the way IN,
 * drawn anyway on the way OUT, retroactively, for the entire run. There is
 * deliberately no direct path here; the test asserts every wallet goes through Relay.
 *
 * That costs a Relay fee per wallet — the honest price of the property — which is why
 * the dust floor exists: below some amount the fee to move a balance exceeds what is
 * being moved.
 *
 * DESTINATION IS THE CALLER'S. 'main' tops the main wallet up for another run;
 * 'treasury' parks the ETH in the wallet that never trades (and then the main wallet
 * is swept too, since it is no longer the destination). ETH ONLY — tokens are the
 * exit's job. One wallet's failure does not stop the others.
 *
 * This is venue-independent (it moves ETH, not tokens), so it is a near-verbatim
 * clone of v3/sweep.js — v6 owns its own copy per the isolation rule.
 */
...
function resolveParties(ks, destination) {
  ...
  const to = destination === 'main' ? v6roles.main(ks) : v6roles.treasury(ks);
  const sources = [...v6roles.bundle(ks)];
...
async function run(userId, input = {}, deps = {}) {
  ...
  if (input.confirm !== true) {
    throw new Error("sweeping moves every v6 wallet's balance — requires { confirm: true }");
  }

  const { to, wallets, skipped } = await plan(userId, input, deps);
  if (!wallets.length) {
    throw new Error(
      'nothing to sweep — every v6 wallet is empty or holds less than the floor. Sell any remaining ' +
  ...
  w.activity(userId).record(
    'v6',
    `[v6] swept ${totals.sent}/${totals.wallets} wallet(s) to the ${to.role === v6roles.ROLES.main ? 'main' : 'treasury'} wallet through Relay` +
