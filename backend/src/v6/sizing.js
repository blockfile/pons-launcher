 ...
 *
 * Pure arithmetic — no provider, no keystore, no network — so its one property
 * (the running mean self-corrects, and the last wallet lands the position on zero)
 * is provable by a test rather than observed on a chain.
 *
 * WHY V6 SIZES BY TOKENS, NOT BY A TARGET ETH AMOUNT (the one real change from v3).
 * v3 trades a constant-product bonding curve, so it can INVERT the curve offline:
 * "sell exactly enough tokens to raise T wei." A Uniswap-V4 pool has concentrated
 * liquidity with no closed-form inverse available off-chain, so v6 cannot ask "how
 * many tokens raise T." Instead it slices the remaining TOKEN balance directly —
 * `remaining tokens ÷ remaining wallets`, jittered — and reads the ETH each sell
 * actually raised from the chain (trade.sell measures a balance delta). The transfer
 * that follows is sized against that real number, exactly as v3's is. Same
 * distribution shape; the venue just answers "what did this raise" live instead of
 * from a formula.
 *
 * THE MEAN IS RECOMPUTED EVERY CYCLE, and that is the whole design. Selling a fixed
 * fraction N times drifts: a slice that hits more price impact than expected leaves
 * the position worth less than the arithmetic assumed, so the run reaches the last
 * wallets with too little. `remaining ÷ walletsLeft` re-derived each cycle spreads a
 * shortfall across the wallets that remain rather than dumping it on the last one.
 *
 * THE LAST WALLET TAKES WHATEVER IS LEFT, exactly — no jitter — so the position ends
 * on zero. The caller passes the whole remaining balance for that cycle.
 */

// How far a cycle's slice may stray from the running mean, and the operator ceiling.
// ±30% is clearly irregular without any single sell standing out; past ±90% the
// small end rounds to dust.
...
function sliceTokens({ tokenBalance, remainingWallets, variancePct = DEFAULT_VARIANCE_PCT, roll = 0.5 }) {
  ...
  // Never more than is there, and never zero — a cycle that sells nothing would
  // transfer nothing and buy nothing, leaving a hole in the tape.
  if (slice >= balance) return balance;
  return slice > 0n ? slice : 1n;
}

/**
 * A LIGHT plan-time feasibility estimate (v6 cannot replay a V4 curve offline the
 * way v3 replays its bonding curve). Given the position's current ETH value (one
 * live quote of selling the whole balance) and the run parameters, it reports the
 * per-wallet average and whether that average clears the per-cycle cost (main-wallet
 * gas + the bundle buy after the Relay fee). This is an ESTIMATE, not a guarantee —
 * every sell moves the price, so the real tail raises less; the engine's per-cycle
 * running mean is what actually self-corrects. Pure arithmetic; the caller formats.
 *
