...
/**
 * V7's Relay client: move ETH from one wallet we hold to another address, on this
 * chain, through a Relay solver.
 *
 * DELIBERATELY A SEPARATE IMPLEMENTATION (not shared with v3/relay.js, v6/relay.js or
 * relay/funding.js). Each tab owns its own money path so the tabs can never break each
 * other; the duplication is the price of that isolation.
 *
 * WHY RELAY AT ALL, WHEN BOTH ENDS ARE ON THE SAME CHAIN: not to bridge — to break the
 * edge. A direct send from the seller to the buyer draws a line anyone reading the chain
 * can follow, and that line is the tell this strategy exists to avoid. With Relay the
 * seller pays a deposit address and a SOLVER pays the buyer — two transactions with no
 * counterparty in common. originChainId and destinationChainId are both config.chainId,
 * an ordinary same-chain order.
 *
 * EXACT_OUTPUT, not EXACT_INPUT: the buyer needs a known amount to buy with, so what
 * matters is what ARRIVES. The fee comes off the sender's side, and the deposit Relay
 * quotes is therefore larger than the amount requested.
 *
 * The flap value currency is native ETH (a native-quoted flap curve pays native out and
 * takes native in), so this native→native Relay client fits V7 verbatim — same as v6.
 */
...
// The same +50% headroom the other relay paths use: Relay's quoted maxFeePerGas can go
// stale between quote and broadcast on this chain, and a stale ceiling gets the deposit
// rejected before it reaches the mempool. Refresh the fee at send time.
...
async function relayRequest(path, { method = 'GET', body, fetchImpl = fetch } = {}) {
  ...
  if (!res.ok) {
    ...
    // never became an order, so a caller may retry it. A 4xx that is not 429 is a real
...
/**
 * Pull the deposit transaction out of a quote, and refuse four ways — cheap checks that
...
/**
 * Relay's own fee fields are DROPPED and re-read from the chain. Relay quotes a concrete
 * maxFeePerGas; on this chain the base fee can tick between the quote and the broadcast,
 * and when it does the deposit is rejected before the mempool. Keep Relay's recipient,
 * value and calldata (the parts that make the order an order); take the ceiling from the
 * chain.
 */
...
async function transfer({ fromWallet, toAddress, amountWei }, deps = {}) {
  ...
  // Retry the QUOTE — and ONLY the quote — through Relay's rate limit. This runs before
  // any deposit is signed or sent, so re-requesting it moves nothing and cannot
  // double-send; a rate-limited cycle recovers on its own instead of halting the run for a
  // manual resume. The deposit broadcast below is NEVER retried.
  const sleepFn = deps.sleepFn || sleep;
  const retries = Math.max(0, Number(deps.quoteRetries ?? config.v7RelayQuoteRetries));
  const backoffMs = Math.max(0, Number(deps.quoteBackoffMs ?? config.v7RelayQuoteBackoffMs));
  let quote;
  ...
  // Checked before signing rather than after a revert, because the caller is a running
  // chain: a cycle that discovers this from a failed broadcast has already sold the tokens
  // that were meant to pay for it.
 ...
 * reported instead of passed off as a success. transfer() returns the moment the deposit
 * is broadcast, but broadcasting is not delivery. Read-only and best-effort: a status
 * endpoint that errors is treated as "not yet", never as a failure of the transfer that
 * already happened.
 *
