...
/**
 * The V7 relay chain: one big buy on a flap BONDING CURVE, then a cycle per bundle
 * wallet of
 *
 *   sell a slice of the position → Relay the proceeds to the next wallet → it buys
 *
 * roughly seven seconds apart, until every wallet has bought. This is v6's engine,
 * unchanged in shape and every safety decision — the venue is the only difference: a
 * flap curve (v7/trade.js), reached through the fixed flap launcher, instead of a
 * letscash Uniswap-V4 pool. Slices are sized in TOKENS and each sale's proceeds are read
 * from the chain (see v7/sizing.js), the value currency is native ETH so the Relay hop
 * fits verbatim, and the sell is two txs instead of three (no Permit2).
 *
 * THE ONE NEW CONCERN v6 NEVER HAD: GRADUATION. A flap token graduates off the curve to a
 * V2 pair when its tokens-sold reaches a threshold. V7 re-checks trade.assertTradable
 * before the big buy and before each bundle buy, so a token that graduates mid-run HALTS
 * AND KEEPS STATE (nothing is signed on the wrong venue) rather than trading a pair V7
 * does not understand. The route's plan-time graduation guard (routes/v7.js) is what stops
 * a big buy sized large enough to graduate the token in the first place.
 *
 * WHY THE TIMER LIVES ON THE SERVER, why the interval is a FLOOR not a clock, why a
 * FAILURE HALTS AND KEEPS STATE, and why RESUME PICKS UP AT THE STEP THAT FAILED (via
 * per-cycle done-flags, so a completed sell+transfer is never re-run and the position
 * never sold twice) — all identical to v3/v6's engine; read v3/engine.js for the
 * reasoning. A restart still loses the in-memory job; nothing is ever left pending (each
 * step awaits its own receipt), so a lost job is a stopped run whose every effect is
 * already on chain.
 */
...
// A deposit to a Relay deposit address is a plain value send; 50k is generous and only
// decides how much a cycle keeps back for its own next deposit's gas.
const RELAY_DEPOSIT_GAS = 50_000n;

// Held back from each transfer for Relay's own fee (charged on the sender's side of an
// EXACT_OUTPUT order, and not known until the quote comes back). Unspent ETH stays in the
 ...
 * against the EXACT figures the engine uses. mainGas covers the main wallet's TWO sell txs
 * (a plain approve + the swapExactInput) plus its next Relay deposit — a flap curve sell is
 * one tx lighter than v6's three-tx V4 sell (no Permit2 leg).
 */
function gasFigures(fees) {
  return {
    buyGas: gasCost(fees, defaultTrade.BUY_GAS),
    buffer: parseEther(String(config.gasBufferEth)),
    mainGas: gasCost(fees, defaultTrade.ERC20_APPROVE_GAS + defaultTrade.SELL_GAS + RELAY_DEPOSIT_GAS),
  };
}
...
function publicJob(job) {
  if (!job) return { protocol: 'v7', mode: 'relay-chain', status: 'idle', running: false, cycles: [] };
  ...
  return {
    ...
    token: job.token,
    venue: job.pool?.venue || null,
    // Curve snapshot taken when the run started (informational; the live "% to
    // graduation" is read fresh by the plan endpoint). state was 0 at start by
    // construction — readCurve refuses anything else.
    curveState: job.pool?.state ?? null,
    circulatingSupplyAtStart: job.pool?.circulatingSupply != null ? job.pool.circulatingSupply.toString() : null,
    dexSupplyThresh: job.pool?.dexSupplyThresh != null ? job.pool.dexSupplyThresh.toString() : null,
    symbol: job.symbol,
...
function createEngine(deps = {}) {
  ...
  function log(userId, summary, detail = {}) {
    activityForFn(userId).record('v7', summary, detail);
  }
  ...
  function schedule(job, delayMs) {
    ...
    job.timer = setTimeoutFn(async () => {
      ...
      try {
        ...
        job.updatedAt = iso(nowFn());
        log(job.userId, `[v7] run halted — ${errorMessage(err)}`, { jobId: job.id, error: errorMessage(err) });
      }
    }, delayMs);
  ...
  /** Retry an IDEMPOTENT read (fees, balance) through a transient blip. NEVER wraps a
   *  broadcast (sell/deposit/buy) or the fill wait — those halt for a checked, manual
   *  resume, because a failure there may have actually landed. */
  ...
  // Waits for the Relay fill to CREDIT the recipient, measured as a rise ABOVE the balance
  // it held before the transfer — never an absolute threshold. V7's bundle wallets are
  // routinely claimed from the seasoning pool and already hold ETH, so an absolute
  // `balance >= needWei` check would return the instant the wallet was armed, masking a
  // deposit that never filled: the buy would then spend the wallet's OWN seasoning ETH
  // while the sold proceeds sit stranded (refundable) at the deposit address, with no halt
  // and no signal. The delta only clears when the fill actually lands. `baseline` is the
  // pre-transfer snapshot; 0n falls back to the absolute check for any caller that has none.
  ...
  async function runBigBuy(job, record) {
    ...
    const main = v7roles.main(ks);

    // Graduation guard: refuse to buy if the token left the state-0 curve since the run
    // began. Throws → halt-and-keep-state (nothing signed) rather than trade the V2 pair.
    await trade.assertTradable(job.token, { ...tradeDeps, rpc });
    ...
    log(job.userId, `[v7] big buy of ${formatEther(job.bigBuyWei)} ETH from the main wallet`, {
      jobId: job.id,
      token: job.token,
      hash: out.hash,
  ...
  async function runCycle(job, index, record) {
    ...
    const ks = keystoreForFn(job.userId);
    const main = v7roles.main(ks);
    const wallet = v7roles.bundle(ks).find((w) => w.id === target.walletId);
    if (!wallet) throw new Error(`wallet ${target.walletId} is no longer a v7 bundle wallet`);
    ...
    // Graduation guard for the SELL (and, on resume where the sell is already done, for the
    // buy that follows): a graduated token can no longer be sold on the curve. Halts here
    // with a clear message instead of burning two txs on a revert.
    await trade.assertTradable(job.token, { ...tradeDeps, rpc });
    ...
    if (!record.sellDone) {
      ...
      // Size the slice in TOKENS. The last wallet takes the whole remaining balance so the
      // position ends on zero — sliceTokens returns the full balance when remainingWallets === 1.
      ...
      // Halt on ANYTHING that is not a confirmed sell — BEFORE marking the step done. A
      // 'reverted' sell moved nothing (safe to resume: it re-sells cleanly). A 'pending'
      // sell is a receipt that did not arrive in time: recording it as done would freeze
      // ethRaised at 0 and every resume would then throw "raised 0 ETH" at the transfer
      // forever — a dead run. Halting here keeps sellDone false, so a resume re-reads the
      // main balance and re-slices. If the pending tx HAD landed, the balance is already
      // lower and the re-slice simply continues from there (no ETH leaves the main wallet
      // either way); if it had not, the retry sells as intended. The operator is told to
      // check the balance first so they can exit instead of resuming when they would rather stop.
      ...
      log(
        job.userId,
        `[v7] cycle ${index}/${job.targets.length}: sold ${
    ...
    // ── transfer ────────────────────────────────────────────────────────────────
    // Sized from what the sell ACTUALLY raised, never the estimate. The main wallet keeps
    // back its own next round of gas and a small Relay-fee allowance.
    if (!record.transferDone) {
      record.step = 'transferring';
      record.state = 'transferring';

      // Snapshot the recipient's balance BEFORE the transfer, so the fill is detected as a
      // delta (see waitForFill). Persisted on the record: if the run halts at the fill step
      // and resumes, the baseline the transfer was measured against survives.
      ...
      log(
        job.userId,
        `[v7] cycle ${index}/${job.targets.length}: transferred ${formatEther(transferWei)} ETH to ${target.address}`,
    ...
    if (!record.buyDone) {
      ...
      // Re-check graduation right before the buy: the token could have graduated during the
      // fill wait. Halts-and-keeps-state, and the fill ETH is safe in the bundle wallet
      // (recoverable by the sweep) rather than spent buying the wrong venue.
      await trade.assertTradable(job.token, { ...tradeDeps, rpc });
      ...
      log(
        job.userId,
        `[v7] cycle ${index}/${job.targets.length}: ${target.address} bought with ${formatEther(spend)} ETH`,
  ...
  function complete(job) {
    ...
    job.nextRunAt = null;
    clear(job);
    log(job.userId, `[v7] run complete — ${job.targets.length} wallet(s) bought`, {
      jobId: job.id,
      token: job.token,
      total: job.targets.length,
    });
  ...
  async function runNext(userId) {
    ...
    try {
      ...
      job.inFlight = false;
      clear(job);
      log(job.userId, `[v7] run halted at ${record.step || record.kind}: ${errorMessage(err)}`, {
        jobId: job.id,
        index,
        walletId: record.walletId,
        step: record.step,
  ...
  /**
   * Validate and begin. The chain-state refusals — is this a genuine state-0 flap curve,
   * is it native-quoted, would the big buy graduate it — belong to the ROUTE, which
   * resolves and verifies the curve (v7/trade.readCurve) and runs the graduation guard
   * before calling here. This checks the run's own shape.
   */
  async function start(userId, input = {}) {
    const existing = jobs.get(userId);
    if (existing?.status === 'running') throw new Error('a v7 chain run is already running');

    const { token, pool, symbol = null, bigBuyWei, targets = [] } = input;
    if (!token || !pool || !pool.venue) {
      throw new Error('token and a resolved flap curve { venue } are required');
    }
    ...
    // The BUY floor. The flap curve is a predictable constant-product, so 0 is allowed (a
    // strictly-guaranteed buy); the default absorbs the fee, on-curve tax and others' impact.
    ...
    if (!Number.isFinite(buySlippageBps) || buySlippageBps < 0 || buySlippageBps > 5000) {
      throw new Error('the buy slippage floor must be between 0 and 5000 bps');
    }
    ...
    log(userId, `[v7] chain started on ${symbol || token} for ${planned.length} wallet(s)`, {
      jobId: job.id,
      token,
      venue: pool.venue,
  ...
  function stop(userId) {
    ...
    job.nextRunAt = null;
    log(userId, `[v7] run stopped at ${job.currentIndex}/${job.targets.length}`, {
      jobId: job.id,
      currentIndex: job.currentIndex,
      total: job.targets.length,
    });
  ...
  function resume(userId) {
    const job = jobs.get(userId);
    if (!job) throw new Error('no v7 chain run to resume');
    if (job.status === 'running') return publicJob(job);
    if (job.status === 'complete') throw new Error('this v7 run is already complete');
    if (job.currentIndex > job.targets.length) throw new Error('this v7 run has no remaining wallets');

    job.status = 'running';
    job.stoppedAt = null;
    job.failure = null;
    job.updatedAt = iso(nowFn());
    log(userId, `[v7] run resumed at ${job.currentIndex}/${job.targets.length}`, {
      jobId: job.id,
      currentIndex: job.currentIndex,
      total: job.targets.length,
    });
