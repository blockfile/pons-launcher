 ...
 * alters what V4 signs, unwatched, on day 11 of a run nobody is watching.
 *
...
// The same headroom v2's funding path (and v3's Relay client) settled on. See
// normaliseTx below for what it is protecting against.
 ...
 *
 * Every one of these has caught something real on the v2/v3 paths. They are
 * cheap, they run before anything is signed, and each names what it found — a
 * quote that fails here is a Relay-side surprise, and the operator needs to
 * know which one rather than reading a signature failure three steps later.
 */
...
/**
 * Send `amountWei` from a funding wallet we hold to a fresh seed wallet,
 * through Relay.
 *
 * @param {object} input
...
async function transfer({ fromWallet, toAddress, amountWei }, deps = {}) {
  ...
  // already burned the gas that was meant to fund a seed wallet.
