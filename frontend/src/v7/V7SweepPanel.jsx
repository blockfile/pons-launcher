 ...
 */
export default function V7SweepPanel({ step, explorer, reload, report, locked }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setPreview(await api(`/v7/sweep/preview?${qs.toString()}`));
    } catch {
  ...
  return (
    <Step {...step}>
      ...
      <Modal
        ...
        onConfirm={async () => {
          await act('sweep', () =>
            api('/v7/sweep', 'POST', { destination, minSweepEth: minSweep || undefined, confirm: true })
          );
          setArming(false);
        }}
      >
