...
import V7TreasuryPanel from './V7TreasuryPanel.jsx';
import V7BundlePanel from './V7BundlePanel.jsx';
import V7MainPanel from './V7MainPanel.jsx';
import V7ChainPanel from './V7ChainPanel.jsx';
import V7ExitPanel from './V7ExitPanel.jsx';
import V7SweepPanel from './V7SweepPanel.jsx';
 ...
 * other, this component holds V7's wallets, its token and its running job, and
 * nothing it does can change what the other tab is drawing.
 *
 * SIX STEPS, AND WHY THERE IS NO LAUNCH AMONG THEM. V7 is not a launcher. It
 * takes a token that is already live — a flap token still on its bonding curve,
 * launched here on another tab, by a wallet this account holds — and
 * distributes a position across bundle wallets one at a time. The launch
 * belongs to whichever tab made it.
 *
 ...
 */
export default function V7Console({ health, credential, report, output, reportedAt }) {
  ...
  const loadWallets = useCallback(async () => {
    try {
      setWallets(await api('/v7/wallets'));
    } catch (err) {
  ...
  // Every V7 wallet the backup would write, for the count on the confirm dialog.
