 ...
 * factory's snipe-tax exemption list and binds only at launch. V7 never
 * launches, so the only cost of more wallets is a longer run and a smaller
 * average buy.
 */
export default function V7BundlePanel({ step, wallets, explorer, reload, report, locked, backupCount }) {
  ...
  /** Hand N seasoned V4 seed wallets over into this v7 bundle (re-roled server-side to v7bundle). */
  async function claimSeasoned() {
    setBusy('claim-seasoned');
    try {
      const n = Math.max(1, Math.round(Number(seasonedCount) || 0));
      const out = await api('/v7/wallets/claim-seasoned', 'POST', { count: n });
      report(
        out.shortfall > 0
  ...
  async function deleteTicked(list) {
    ...
    for (const w of list) {
      ...
      try {
        await api(`/v7/wallets/${w.id}`, 'DELETE');
        done += 1;
      } catch (err) {
  ...
  return (
    <Step {...step}>
      ...
      <Modal
        ...
        onConfirm={async () => {
          await act('delete', () => api(`/v7/wallets/${deleting.id}`, 'DELETE'));
          setDeleting(null);
        }}
      >
        <p>
