 ...
 *
 * The keystore permits exactly one v6dev, so replacing this key after an
 * exposure means deleting the one that is there. That control lives beside the
 * wallet it deletes.
 */
export default function V6TreasuryPanel({ step, wallet, explorer, reload, report, locked, backupCount }) {
  ...
  return (
    <Step {...step}>
      ...
      {wallet ? (
        ...
        <div className="row">
          <Busy
            ...
            onClick={() =>
              act('generate', () =>
                api('/v6/wallets/generate', 'POST', { count: 1, role: ROLES.treasury, label: 'v6 treasury' })
              )
            }
          >
      ...
      <Modal
        ...
        onConfirm={async () => {
          await act('delete', () => api(`/v6/wallets/${wallet.id}`, 'DELETE'));
          setDeleting(false);
        }}
      >
        <p>
