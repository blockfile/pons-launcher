 ...
 * Step 5 — the exit.
 *
 * Sells every V6 wallet's whole balance, INCLUDING the main wallet's. That last
 ...
 */
export default function V6ExitPanel({ step, token, setToken, explorer, reload, report, locked }) {
  ...
  return (
    <Step {...step}>
      <p className="lede">
        Empties every V6 wallet of this token — the bundle wallets and the main wallet, which
      ...
      {preview && preview.wallets.length > 0 ? (
              ...
              <tbody>
                {preview.wallets.map((w) => (
                  <tr key={w.walletId}>
                    ...
                    <td>{w.role === 'v6main' ? 'main' : 'bundle'}</td>
        ...
        <p className="hint">
          {token?.trim()
            ? 'No V6 wallet holds any of this token yet.'
      ...
      <Modal
        ...
        onConfirm={async () => {
          await act('sell', () => api('/v6/exit', 'POST', { token: token.trim(), confirm: true }));
          setArming(false);
        }}
      >
        <p>
          Every wallet approves exactly its balance and sells it into the pool at whatever price it
