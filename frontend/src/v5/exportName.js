/**
 * The filename of a V5 key export: {count}pcs-V5-{what}[-{suffix}]-{date}.{ext},
 * e.g. 30pcs-V5-bundle-wallets-2026-09-11.json.
 *
 ...
 *
 * V5's own copy, per the tab-isolation rule: every tab has one. The role strings
 ...
 */
const ROLE_WORD = { v5dev: 'dev', v5bundle: 'bundle' };

export function v5ExportName({ wallets = [], qualifier = '', suffix = '', ext = 'json', date = new Date() } = {}) {
  ...
  return `${wallets.length}pcs-V5-${what}${suffix ? `-${suffix}` : ''}-${date.toISOString().slice(0, 10)}.${ext}`;
}
